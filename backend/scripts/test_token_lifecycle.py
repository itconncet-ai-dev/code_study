...
async def test_token_lifecycle():
    ...
    expires_at = datetime.now(timezone.utc) + settings.refresh_token_lifetime
    ...
    try:
        payload = verify_token(access_token, expected_type=TokenType.ACCESS, settings=settings)
        ...
        print(f"   - remaining time: {remaining.total_seconds():.0f}s ({remaining.total_seconds()/60:.1f}min)")
    ...
    if stored_refresh_token is not None:
        ...
        if stored_refresh_token.user_id == test_user_id:
            ...
            print(f"   [FAIL] user_id does not match!")

        # Validity check
        if stored_refresh_token.is_valid():
            print(f"   [PASS] Token is valid (not expired, not revoked)")
        else:
            print(f"   [FAIL] Token is not valid")

        # Remaining validity
        remaining = stored_refresh_token.expires_at - datetime.now(timezone.utc)
